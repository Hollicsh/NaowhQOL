...
-- Synchronous unit scanning and buff comparison
    ...
    for _, buff in ipairs(Categories.RAID) do
        ...
        elseif not BWV2:HasClassInGroup(buff.class) then
            ...
            for unit, data in pairs(BWV2.raidResults) do
                -- Skip "player" key in raid mode (player already counted via raidX)
...
function Scanner:ScanClassBuffs()
    ...
    for _, group in ipairs(classData.groups or {}) do
        ...
        if shouldCheck then
            ...
            -- Check duration threshold if configured (skip for weaponEnchant)
            local remaining = nil
                ...
                if threshold > 0 then
                    ...
                    if buffData and buffData.expiry and buffData.expiry > 0 then
                        ...
                        if remaining < threshold then
                            hasBuff = false  -- Below threshold = failing
                        end
                    end
                end
            end
...
function Scanner:ScanConsumables()
    ...
    for _, buff in ipairs(Categories.CONSUMABLE_GROUPS) do
        -- Build merged spell IDs list (defaults + user added)
        local spellIDs = {}
        ...
        elseif primaryID and Categories:IsDefaultDisabled("consumable_" .. buff.key, primaryID) then
            ...
            if not skip then
                ...
                -- Normal spell check (uses merged spellIDs from defaults + user entries)
...
-- Scan a single unit and store results
...
-- Perform a full synchronous scan of all units
