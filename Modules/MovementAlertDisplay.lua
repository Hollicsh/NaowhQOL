...
--- Burning Rush Reminder pattern: track auraInstanceID from UNIT_AURA payloads (works in combat).
...
local function CacheMovementSpells(fullReset)
    ...
    for _, entry in ipairs(cachedMovementSpells) do
        ...
        if C_Spell.GetOverrideSpell then
            ...
            if okOvr and oid and oid > 0 and oid ~= entry.spellId then
    ...
    if specAbilities then
        for _, spellId in ipairs(specAbilities) do
            ...
            elseif not trackedSpellSet[spellId] then
                if C_Spell.GetOverrideSpell then
                    local okOvr, oid = pcall(C_Spell.GetOverrideSpell, spellId)
                    if okOvr and oid and oid > 0 and oid ~= spellId and trackedSpellSet[oid] then
...
local function MovementHasActiveBuffActive()
    for _, entry in ipairs(cachedMovementSpells) do
        if entry.checkType == "buffActive" then
            local state = buffActiveState[BuffActiveKey(entry)]
            if state and state.active then return true end
        end
    end
    return false
...
local function MovementShouldPollBuffAuras()
    ...
    if MovementHasActiveBuffActive() then return true end
...
local function CheckGatewayUsable()
    ...
    if not ok then itemCount = 0 end
...
loader:SetScript("OnEvent", ns.PerfMonitor:Wrap("Movement Alert", function(self, event, ...)
    ...
    elseif event == "UNIT_AURA" then
        ...
        if unit == "player" then
            OnPlayerBuffActiveAuraUpdate(updateInfo)
        end
        UpdateCachedCharges()
        CheckMovementCooldown()
